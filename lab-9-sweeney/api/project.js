'use strict';
//collecting the necessary functions to execute commands facilitated by the index.js hub. review CRUD/api request flow chart
...
// calling seed function from storage js
storage.seed();
...
function createProject(req,res){
  bodyParser(req).then((body)=>{
    try{
      ...
      let message = JSON.stringify({ error : err,});
  ...
  bodyParser(req).then((body)=>{
    try{
      ...
      if(body.id !== undefined){
        let id = body.id;
        let project = storage.update(id, name, description, url).then(project =>
        {
          responses.text200(req, res, `project update success ${project.id}`);
        });
      }
    } catch(err){
  ...
  if(req.url.pathname === '/api/projects'){
    ...
    if(req.url.query.id){
      ...
      let removed = storage.remove(id)
        .then((json)=>{
          response.text204(req, res, `Success ${id}`);
        });
    }else{
      let projects = storage.getAll();
